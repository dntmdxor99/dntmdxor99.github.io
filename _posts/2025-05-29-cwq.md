...
Weight-only quantization is a promising technique that not only significantly reduces the memory footprint of large language models (LLMs) but also enables faster inference. By converting traditional floating-point parameters into low-bit integer-based parameters, this approach drastically reduces memory requirements and alleviates memory-bound overheads that often hinder acceleration. Recent advancements have demonstrated that models can be quantized down to 4-bit precision while maintaining accuracy comparable to their 16-bit floating-point precision. Furthermore, extreme low-bit quantization—such as 2-bit and 3-bit—can achieve even greater compression with only a tolerable level of performance degradation. Prominent examples of discrete weight-only quantization methods include GPTQ, AWQ, and HQQ, which are widely used in practice.

However, as LLM model sizes continue to grow while hardware advancements remain relatively slow, there is still a need for methods that maximize device resource utilization while minimizing performance degradation. For instance, Llama 2 70B requires 140GB of memory in its full-precision form, whereas it occupies only 17.5GB and 26.25GB under 2-bit and 3-bit quantization, respectively. Therefore, in order to load such models on a single RTX 4090 GPU with 24GB of memory, aggressive quantization—such as 2-bit—is often necessary, even at the cost of noticeable performance degradation.
...
To overcome these challenges, we propose a new method that enables optimal bit allocation based on a principled criterion within a single forward pass. First, we define \textbf{a quantization proxy sensitivity} metric that eliminates the need to fully quantize the model at each search iteration, while still effectively representing the layer-wise quantization error. Second, we utilize the proxy sensitivities for 2, 3, and 4-bit quantization to \textbf{construct a bit-performance Pareto frontier}, from which bit combinations suitable for various deployment constraints can be derived. Building on these ideas, we introduce \textbf{CWQ: Continuous Relaxation for Weight-only Quantization}, a novel and computationally efficient method that bridges the gap between discrete and continuous weight-only quantization for large-scale language models.
