...
<div class="caption">
    Figure 3. Loss degradation when each linear layer of Llama-2-7B is individually quantized to 2 bits (others at 4 bits). Sensitivity varies widely, motivating selective high-precision allocation.  ￼
</div>

As Figure 3 demonstrates, certain layers (e.g., Value and down projections) extremely increase loss under 2-bit, whereas others show minimal impact.  This loss varying validates our strategy allocate high-precision to high-sensitivity layers.

---

## Loss-based Dynamic Bit Allocation

To optimally distribute a fixed average bit-width budget B across layers, we exploit the layer-wise quantization sensitivity measured via Jensen–Shannon divergence (JSD) as a loss proxy.
	1.	Sensitivity Measurement.
Using our quantization proxy—which assembles pre-quantized 2-bit and 4-bit replicas of each linear layer in O(1) time—we individually quantize layer i to 2-bit (all others at 4-bit) and compute the JSD between its output distribution and the FP16 baseline on a small calibration set.  Layers whose JSD exceeds twice the median are labeled sensitive and are fixed at 4-bit to preserve model quality  ￼.
	2.	CWQ Allocation.
Denote the set of sensitive layers by \mathcal{S}.  CWQ achieves the target average B by assigning 4-bit to all i\in\mathcal{S} and 2-bit to every other layer.  This simple binary assignment matches B in closed form, eliminating the need for expensive search  ￼.
	3.	CWQ+ Grid-Search Refinement.
To further reduce the proxy loss, CWQ+ allows a third precision level (3-bit).  We perform a lightweight grid search over the number k of non-sensitive layers to promote from 2- to 3-bit, evaluating each candidate via the quantization proxy and selecting the k that minimizes aggregate JSD while exactly satisfying B.  Since the proxy assembly is O(1) per layer, this incurs only O(n) overhead for n layers  ￼.
	4.	Final Data-Dependent Quantization.
With bit-widths fixed per layer, we apply a state-of-the-art, data-dependent quantization method (e.g., AWQ) independently to each layer at its assigned precision.  This last step leverages activation-aware clipping to further reduce quantization error without altering the allocation scheme  
